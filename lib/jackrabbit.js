...
const jackrabbit = (url, logger, options = {}) => {
    ...
    options.reconnectionTimeout = options.reconnectionTimeout || +process.env.RABBIT_RECONNECTION_TIMEOUT || 3000;
    options.maxRetries = options.maxRetries || +process.env.RABBIT_RECONNECTION_RETRIES || 99999999;
    ...
    const createDirectExchange = (name, directExchangeOptions = {}) => {

        const mergedOptions = {
            noReply: false, ...directExchangeOptions
        };
        return createExchange()('direct', name, mergedOptions);
    };
